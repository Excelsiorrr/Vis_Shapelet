...
# 1. 定义一个你本地的保存路径 (建议使用相对路径，保存到当前项目的 datasets 目录下)
        # 这样会在你的 D:\shapelet\ShapeX\ 下创建一个 datasets 文件夹
        save_dir = os.path.join('.', 'datasets', 'Spike', 'spike_null')
        
        # 2. 关键步骤：如果没有这个文件夹，就自动创建它！
        os.makedirs(save_dir, exist_ok=True)
        
        # 3. 拼接完整的文件名
        save_path = os.path.join(save_dir, 'spike_split={}.pt'.format(i + 1))

        # 4. 保存
        torch.save(dataset, save_path)
        print(f"Saved to: {save_path}") # 打印一下保存位置，确认成功
