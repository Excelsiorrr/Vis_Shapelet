...
def normalize_exp(exps):
    ...
    # Batch-first (N,T,D): normalize each sample across its time/channel axes
    for i in range(exps.shape[0]):
        sample = exps[i,:,:]
        norm_exps[i,:,:] = (sample - sample.min()) / (sample.max() - sample.min() + 1e-9)
    return norm_exps
...
def ground_truth_xai_eval(generated_exps, gt_exps, penalize_negatives = True, times = None):
    ...
    for i in range(generated_exps.shape[0]):
        # Batch-first indexing: sample i
        if times is not None:
            mask = times[i,:] > -1e5
            gte = (gt_exps[i,:,:][mask]).flatten()
            gene = normalize_one_exp(generated_exps[i,:,:][mask]).flatten()
